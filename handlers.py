...
1️⃣ <b>Загрузить своё видео или кружок</b> — просто отправьте видео/кружок боту (если одобрено — +1 к лимиту)
...
async def cmd_start(message: Message, bot_username: str, admin_id: int):
    ...
    if user_id == admin_id:
        await message.answer(
            ...
            f"🗑 /delrange <code>start-end</code> — удалить несколько видео (например: 750-754)\n"
...
async def get_next_video(event, admin_id: int):
    ...
    if available_videos <= 0:
        await message.answer(
            ...
            f"1️⃣ <b>Загрузите своё видео или кружок</b> — отправьте видео/кружок боту (если одобрено — +1)\n"
    ...
    try:
        ...
        # Если видео удалено из чата админа - удаляем его из списка и пропускаем
        if "message to copy not found" in str(e).lower() or "message_id_invalid" in str(e).lower():
            ...
            # Пробуем получить следующее видео
            await message.answer(
                f"⚠️ <b>Видео #{next_video['id']} недоступно</b>\n\n"
